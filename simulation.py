...
if st.button("Run Simulation"):
    ...
    with st.spinner(f"Fetching historical data for {ticker} from {exchange_id}..."):
        try:
            ...
            # Use a generous limit, as fetch_ohlcv might be limited per call
            ohlcv = exchange.fetch_ohlcv(ticker, timeframe, limit=1000) # Fetch up to 1000 candles

            if not ohlcv:
                st.error(f"No historical data fetched for {ticker} from {exchange_id}. Check ticker symbol or exchange status.")
                st.stop()
            ...
            # CCXT usually returns data in ascending order by default, but sorting is safe.
    ...
    # Crypto markets are 24/7, so using 'D' (calendar days) might be more appropriate than 'B' (business days).
    # However, 'B' looks better on a chart with distinct steps, let's stick to 'B' for visual consistency with stock charts.
    # If you prefer calendar days for crypto, change 'B' to 'D'.
    ...
    if len(simulated_dates) != simulation_days:
         st.warning(f"Could not generate {simulation_days} business days after {last_historical_date.strftime('%Y-%m-%d')}. Generated {len(simulated_dates)}. Trimming simulation results.")
